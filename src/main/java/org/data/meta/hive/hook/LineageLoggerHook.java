    ...
    public void run(final HookContext hookContext) {
        ...
        if (ss != null && index != null && LineageLoggerHook.OPERATION_NAMES.contains(plan.getOperationName()) && !plan.isExplain()) {
            try {
                ...
                log("获取查询时参数："+hookContext.getConf().getQueryString());
                log("获取指定的参数"+hookContext.getConf());
//                EventEmitterFactory.get().emit("###########@@@@@@@@###############@@@@@@@@@@@@@@@@@@@@@@@@@@@@@########################");
//                EventEmitterFactory.get().emit("index:"+edges.toString());
//                EventEmitterFactory.get().emit("###########@@@@@@@@###############@@@@@@@@@@@@@@@@@@@@@@@@@@@@@########################");

                final List<FineGrainedLineages> fineGrainedLineages = this.buildFineGrainedLineages(edges);
                final List<Upstreams> upstreams = this.buildUpstreams(edges);
                //写入mysql数据库
                final Value value = new Value();
                value.setFineGrainedLineages(fineGrainedLineages);
                value.setUpstreams(upstreams);
                ...
                proposal.setEntityUrn(buildEntityUrn(edges));
                ...
                mysqlEntity.setEntityUrn(buildEntityUrn(edges));
    ...
    private List<FineGrainedLineages> buildFineGrainedLineages(final List<Edge> edges) {

        final List<FineGrainedLineages> fineGrainedLineages = new ArrayList<FineGrainedLineages>();

        for (final Edge edge : edges) {
            if (edge.type == Edge.Type.PREDICATE){
                continue;
            }
            final ArrayList<String> upstreams = new ArrayList<>();
            final ArrayList<String> downstreams = new ArrayList<>();
            ...
            for (final Vertex vertex : edge.sources){
                ...
                final String srcColumnName ;
                srcDatabase   = MetaLogUtils.normalizeIdentifier(vertex.dbName);
                srcTableName  = MetaLogUtils.normalizeIdentifier(vertex.tableName);
                srcColumnName = MetaLogUtils.normalizeIdentifier(vertex.columnName);
                upstream = "urn:li:schemaField:(urn:li:dataset:(urn:li:dataPlatform:hive," + srcDatabase + "."+ srcTableName
                        + ",PROD)," + srcColumnName + ")" ;
            ...
            for (final Vertex vertex : edge.targets){
                ...
                final String srcDatabaseTarget ;
                final String srcTableNameTarget ;
                final String srcColumnNameTarget ;
                srcDatabaseTarget   = MetaLogUtils.normalizeIdentifier(vertex.dbName);
                srcTableNameTarget  = MetaLogUtils.normalizeIdentifier(vertex.tableName);
                srcColumnNameTarget = MetaLogUtils.normalizeIdentifier(vertex.columnName);
                downstream = "urn:li:schemaField:(urn:li:dataset:(urn:li:dataPlatform:hive," + srcDatabaseTarget + "."+ srcTableNameTarget
                        + ",PROD)," + srcColumnNameTarget + ")" ;
                downstreams.add(downstream);
            }
            FineGrainedLineages fineGrainedLineage = new FineGrainedLineages();
            fineGrainedLineage.setUpstreams(upstreams);
            fineGrainedLineage.setDownstreams(downstreams);
    ...
    private List<Upstreams> buildUpstreams(final List<Edge> edges) {

        final List<Upstreams>  upstreams= new ArrayList<Upstreams>();
        for (final Edge edge : edges) {
            if (edge.type == Edge.Type.PREDICATE){
                continue;
            }
            ...
            for (final Vertex vertex : edge.sources){
                final Upstreams upstream = new Upstreams();
                srcDatabase   = MetaLogUtils.normalizeIdentifier(vertex.dbName);
                srcTableName  = MetaLogUtils.normalizeIdentifier(vertex.tableName);
                dataset = "urn:li:dataset:(urn:li:dataPlatform:hive," + srcDatabase + "."+ srcTableName
                        + ",PROD)";
                AuditStamp auditStamp = new AuditStamp();
                upstream.setAuditStamp(auditStamp);
                upstream.setDataset(dataset);
                upstreams.add(upstream);
            }
    ...
    private String buildEntityUrn(final List<Edge> edges) {
        ...
        for (final Edge edge : edges) {
            ...
            for (final Vertex vertex : edge.targets){
                srcDatabase   = MetaLogUtils.normalizeIdentifier(vertex.dbName);
                srcTableName  = MetaLogUtils.normalizeIdentifier(vertex.tableName);
                entityUrn = "urn:li:dataset:(urn:li:dataPlatform:hive," + srcDatabase + "."+ srcTableName
                        + ",PROD)" ;
            }
        }
        return entityUrn;
